...
def main():
    ...
    trainloader = DataLoader(tokenized_dataset['train'], batch_size=args.per_gpu_batch_size, shuffle=True, collate_fn=collate_func, num_workers=args.num_workers)
    testloader = DataLoader(tokenized_dataset['validation'], batch_size=args.per_gpu_batch_size, shuffle=False, collate_fn=collate_func, num_workers=args.num_workers)
    ...
    scheduler = get_cosine_schedule_with_warmup(optimizer,
                                                ...
                                                num_training_steps=args.epochs*len(trainloader)*accelerator.num_processes)
    ...
    if args.loading_from_checkpoint:
        ...
        checkpoints_sorted = sorted(filter(lambda x: weights_type in x, os.listdir(path)), key=lambda x: int(x.split('_')[-1].split('.')[0]))
    ...
    ## accelerate.prepare
    model, optimizer, scheduler, trainloader, testloader = accelerator.prepare(model, optimizer, scheduler, trainloader, testloader)
        ...
        if accelerator.is_main_process:
            ...
            if args.use_lora:
                if epoch+1 == args.epochs:
                    accelerator.unwrap_model(model).save_weights(os.path.join(path_to_save, f'checkpoint_merged_{epoch+1}.safetensors'))
                accelerator.unwrap_model(model).save_weights(os.path.join(path_to_save, f'checkpoint_lora_{epoch+1}.safetensors'), merge_weights=False)
            elif not args.use_lora and args.train_head_only:
                save_file(accelerator.unwrap_model(model).state_dict(), os.path.join(path_to_save, f'checkpoint_headonly_{epoch+1}.safetensors'))
            else:
                save_file(accelerator.unwrap_model(model).state_dict(), os.path.join(path_to_save, f'checkpoint_fulltrain_{epoch+1}.safetensors'))
